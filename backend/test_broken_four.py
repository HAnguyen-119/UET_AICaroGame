...
class Utility:
   ...
   def five_utility(self):
      ...
      for line in FIVES:
         ...
         if self.counter(line_val).get(' ', 0) == 1:
            if self.counter(line_val).get('x', 0) == 4:
               brf[0] += 1
            elif self.counter(line_val).get('o', 0) == 4:
               brf[1] += 1

      return thr, two, brf  # three, two, brt
